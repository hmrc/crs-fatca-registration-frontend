...
object IndividualContactPhoneSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): SummaryListRow = {
    val indContactPhone = answers.get(IndividualPhonePage).getOrElse(messages("checkYourAnswers.notProvided"))
    SummaryListRowViewModel(
      key = s"$IndividualPhonePage.checkYourAnswersLabel",
      value = ValueViewModel(HtmlFormat.escape(indContactPhone).toString),
      actions = Seq(
        changeAction(

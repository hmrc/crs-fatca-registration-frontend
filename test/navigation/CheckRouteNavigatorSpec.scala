    ...
    "in Check mode" - {
      ...
      "must go from BusinessTradingNameWithoutIDPage to NonUKBusinessAddressWithoutIDPage if there is no address" in {
        navigator
          ...
          .mustBe(controllers.organisation.routes.NonUKBusinessAddressWithoutIDController.onPageLoad(NormalMode)) // NM
      }
      ...
      "must go from IndNonUKAddressWithoutIdPage to IndContactEmailPage if there is no contact email" in {
        navigator
          ...
          .mustBe(controllers.individual.routes.IndContactEmailController.onPageLoad(NormalMode)) // NM
      }
      ...
      "must go from IndDateOfBirthPage to IndIdentityConfirmedPage if Nino" in {
        ...
        navigator.nextPage(IndDateOfBirthPage, NormalMode, userAnswers) mustBe IndIdentityConfirmedController.onPageLoad(NormalMode) // NM
      }

...
import pages.changeContactDetails.{OrganisationContactNamePage, OrganisationPhonePage}
import play.api.inject.bind
import play.api.inject.guice.GuiceApplicationBuilder
import play.api.test.FakeRequest
import play.api.test.Helpers._
import uk.gov.hmrc.auth.core.AffinityGroup
import views.html.changeContactDetails.OrganisationPhoneView

import scala.concurrent.Future

class OrganisationPhoneControllerSpec extends SpecBase with MockitoSugar {

  val formProvider = new OrganisationPhoneFormProvider()
  val form         = formProvider()

  val contactName = "John Doe"

  lazy val organisationPhoneRoute = controllers.changeContactDetails.routes.OrganisationPhoneController.onPageLoad(NormalMode).url
  ...
  "OrganisationPhone Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[OrganisationPhoneView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId)
        .set(OrganisationPhonePage, "answer")
        .success
        .value
        .set(OrganisationContactNamePage, "John Doe")
        .success
        .value
    ...
    "must redirect to the next page when valid data is submitted and affinityGroup is Organisation" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, controllers.changeContactDetails.routes.OrganisationPhoneController.onSubmit(NormalMode).url)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val application = applicationBuilder(userAnswers = Some(UserAnswers(userAnswersId).set(OrganisationContactNamePage, contactName).success.value)).build()

      running(application) {
        val request =
          FakeRequest(POST, organisationPhoneRoute)

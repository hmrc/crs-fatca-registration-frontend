...
import models.{IndContactName, NormalMode, UserAnswers}
...
class IndContactNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IndContactName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(IndContactName("value 1", "value 2")), NormalMode)(request, messages(application)).toString
      }
    }

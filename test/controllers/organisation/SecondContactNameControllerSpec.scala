...
import views.html.organisation.SecondContactNameView

import scala.concurrent.Future

class SecondContactNameControllerSpec extends SpecBase with MockitoSugar with UserAnswersGenerator {
      ...
      forAll(orgWithId.arbitrary) {
        (userAnswers: UserAnswers) =>
          ...
          running(application) {
            ...
            val updatedForm = userAnswers.get(SecondContactNamePage).map(form.fill).getOrElse(form)

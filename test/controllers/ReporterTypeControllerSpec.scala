...
class ReporterTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ReporterType Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ReporterTypePage, ReporterType.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(ReporterType.values.head), NormalMode)(request, messages(application)).toString
      }
    }
